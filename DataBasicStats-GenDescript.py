...
base_path = "all-rnr-annotated-threads"

for f in folds:
    path1 = os.path.join(base_path, f, "rumours")
    for dir1 in os.listdir(path1):
        if '_' in dir1:
            continue   # skip weird dirs

        # ---- annotation for the thread (label) ----
        path_target  = os.path.join(path1, dir1, "annotation.json")
        with open(path_target, "r", encoding="utf-8", errors="replace") as file:
            data_ann = json.load(file)

        target = convert_annotations_data(data_ann)
        y.append(target)
        ...
        for dir2 in os.listdir(path2):
            ...
            with open(path3, "r", encoding="utf-8", errors="replace") as file:
                data = json.load(file)
            ...
            has_url = len(data["entities"]["urls"]) > 0   # True/False
            ...
            events.append(f)
            thread_ids.append(dir1)
            tweet_ids.append(os.path.splitext(dir2)[0])  # remove .json if present
...
df = pd.DataFrame({
    ...
    "followings": tweets_count,
    "hashtag": hashtag,
    "urls": url,
    "target": y,     # from convert_annotations_data
})

df = df.infer_objects()
df.to_csv("CSV_Dataset.csv", index=False)
print("Saved CSV_Dataset.csv")
